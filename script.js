...
function renderResult() {
  ...
  Object.entries(categories).forEach(([cat, scores]) => {
    ...
    if (total >= 65) {
      riskLabel = "（風險等級：低）"; lowCount++;
    } else if (total >= 45) {
      riskLabel = "（風險等級：中）"; midCount++;
    } else {
      riskLabel = "（風險等級：高）"; highCount++;
    }

    if (cat === "心理穩定性") {
      text = total >= 65 ? "您具備高抗壓與精神穩定性，適合進入高互動節點。" :
             total >= 45 ? "整體心理穩定性尚可，建議限制旅程時長並搭配情緒緩衝模組。" :
             "心理穩定度偏低，不建議接入旅程。建議接受進一步意識穩定訓練。";
    }
    if (cat === "記憶與意識邊界感") {
      text = total >= 65 ? "記憶區分與自我感強烈，可承受深層角色記憶植入與同步體驗。" :
             total >= 45 ? "偶有邊界模糊情形，建議搭配認知屏障模組使用。" :
             "邊界感薄弱，容易產生身份混淆與記憶漂移，不建議植入他人記憶。";
    }
    if (cat === "道德彈性與自我認同") {
      text = total >= 65 ? "道德與身份結構穩定，可承擔多重角色模擬任務。" :
             total >= 45 ? "認同感略有漂移，建議避開高衝突任務與倫理模擬節點。" :
             "道德界線與自我結構極不穩定，不建議用於任何載體。";
    }
    if (cat === "壓力適應力與情緒彈性") {
      text = total >= 65 ? "具備良好情緒彈性，適合進入哲學性與高壓節點任務。" :
             total >= 45 ? "可適應常規壓力，但建議避免暴露於過度情緒環境。" :
             "壓力耐受度低，建議延後旅程並接受情緒結構強化訓練。";
    }
    if (cat === "身份邊界與意識界線穩定度") {
      text = total >= 65 ? "自我邊界清晰，適合執行身份變化與角色重構任務。" :
             total >= 45 ? "存在角色融入傾向，建議限制多重模擬與高仿真環境。" :
             "身份解離風險高，請暫緩作為載體，並接受深層回歸訓練。";
    }
  ...
  report.innerHTML += `
    ...
    <p style="line-height:1.8;">
      本報告由夢航公司意識對映模組 EULYSIA™ 協同生成，根據您的心理答題結果進行多維度風險評估與適配推算。本評估不僅反映您目前的情緒韌性與人格結構，也揭示了您在「作為未來人載體」時可能面臨的身心挑戰。<br><br>
      時空旅行雖為 2201 年的重要文明活動之一，但作為原木載體的人類，仍需承擔由他者意識進入體內所產生的壓力與重組風險。我們深知，每一個人的神經網絡皆獨特而脆弱，因此在開放身體共享之前，必須進行嚴格的心理安全審查。<br><br>
      您的本次表現為旅程設計與節點選擇提供了具體依據，未來也將由 EULYSIA 根據此次評估結果擬定推薦訓練模組或安全上限規格。<br><br>
